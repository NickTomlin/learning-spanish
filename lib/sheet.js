...
export function normalizeSheet(sheet) {
  ...
  const quizType = sheet.quizType || 'typing';
  if (!['typing', 'multiple-choice'].includes(quizType)) {
    throw new Error(`Sheet "${sheet.id}" has unknown quizType "${quizType}"`);
  }
  ...
  const axis = {
    ...
    ids: (sheet.axis.ids || sheet.axis.values.map((_, index) => index)).map(String),
  };
  const width = axis.values.length;
  if (axis.ids.length !== width || new Set(axis.ids).size !== width) {
    throw new Error(`Sheet "${sheet.id}" must have one unique axis id per value`);
  }
...
const AXIS_COLOR_CLASSES = {
  yo: 'pronoun-yo',
  tu: 'pronoun-tu',
  'el-ella-usted': 'pronoun-third',
  nosotros: 'pronoun-nosotros',
  'ellos-ustedes': 'pronoun-they',
};

export function axisClass(axis, index) {
  return AXIS_COLOR_CLASSES[axis.ids[index]] || '';
}

export function axisClassForLabel(axis, label) {
  const index = axis.values.indexOf(label);
  if (index !== -1) return axisClass(axis, index);

  const shortIndex = axis.shortValues.indexOf(label);
  return shortIndex === -1 ? '' : axisClass(axis, shortIndex);
}
...
/** Build unique answer choices, preferring distractors from the active pool. */
export function multipleChoiceAnswers(
  question,
  { questions, fallbackQuestions = questions, count = 4, random = Math.random } = {}
) {
  const choices = [];
  const seen = new Set();
  const add = (candidate) => {
    if (
      question.item.distractorGroup
      && candidate.item.distractorGroup === question.item.distractorGroup
    ) return;

    const key = normalizeAnswer(candidate.answer);
    if (!key || seen.has(key)) return;
    seen.add(key);
    choices.push(candidate.answer);
  };

  seen.add(normalizeAnswer(question.answer));
  choices.push(question.answer);
  for (const candidate of shuffle(questions || [], random)) add(candidate);
  for (const candidate of shuffle(fallbackQuestions || [], random)) add(candidate);
...
export function weightedPick(items, weightFor, random = Math.random) {
  if (!items.length) return undefined;

  const weights = items.map(weightFor);
  if (weights.some((weight) => !Number.isFinite(weight) || weight <= 0)) {
    throw new Error('Quiz weights must be positive numbers');
  }

  let target = random() * weights.reduce((total, weight) => total + weight, 0);
