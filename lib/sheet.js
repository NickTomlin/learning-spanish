...
export function allQuestions(sheet, categoryIds) {
  const allowed = categoryIds ? new Set(categoryIds) : null;
  const out = [];
  sheet.items.forEach((item, itemIndex) => {
    if (allowed && !allowed.has(item.category)) return;
 ...
 */
export function sampleQuestions(sheet, { categoryIds, count, random = Math.random, spread = false } = {}) {
  const pool = allQuestions(sheet, categoryIds);
