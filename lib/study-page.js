    ...
    <section data-panel="quiz" class="panel">
      <div class="quiz-panel">
        <div class="quiz-controls">
          ...
          <button class="quiz-settings" data-ref="open-settings">Quiz options ↓</button>
        </div>
        <div class="quiz-card">
          <div class="q-prompt">
          ...
          <div class="accent-bar" data-ref="accent-bar">
            ${SPANISH_CHARACTER_KEYS.map(({ character, shortcut }) => `
              <button data-c="${character}" title="${character} — Ctrl/Cmd+Enter, then ${shortcut}">
...
function mountQuiz(sheet, root) {
  ...
  let activeCategories = loadCategories(categoryStorageKey, sheet);
  let activeAxisIndices = loadAxisIndices(axisStorageKey, sheet);
  ...
  function pickQuestion() {
    const pool = allQuestions(sheet, [...activeCategories], [...activeAxisIndices]);
  ...
  ref(root, 'cat-close').addEventListener('click', () => {
    ...
    activeCategories = new Set(
      checkedCategories.length ? checkedCategories : sheet.categories.map((c) => c.id)
    );
    activeAxisIndices = new Set(
      checkedAxisIndices.length ? checkedAxisIndices : sheet.axis.values.map((_, index) => index)
    );
...
function loadAxisIndices(key, sheet) {
  const all = sheet.axis.values.map((_, index) => index);
  try {
    const saved = JSON.parse(localStorage.getItem(key));
    const valid = Array.isArray(saved) ? saved.filter((index) => all.includes(index)) : [];
    if (valid.length) return new Set(valid);
  } catch { /* fall through to "everything selected" */ }
  return new Set(all);
}
