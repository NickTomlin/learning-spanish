...
function mountQuiz(sheet, root) {
  ...
  let activeAxisIndices = readUrlSelection('axis', axisIndices, Number)
  ...
  ref(root, 'cat-close').addEventListener('click', () => {
    ...
    writeQuizOptionsToUrl(activeCategories, categoryIds, activeAxisIndices, axisIndices);
...
function writeQuizOptionsToUrl(categories, allCategories, axisIndices, allAxisIndices) {
  const url = new URL(window.location.href);
  writeUrlSelection(url.searchParams, 'cats', categories, allCategories);
  writeUrlSelection(url.searchParams, 'axis', axisIndices, allAxisIndices);
  window.history.replaceState({}, '', url);
}
