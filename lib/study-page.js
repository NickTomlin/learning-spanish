    ...
    <div class="table-wrap">
      <table class="ref-table">
        <thead><tr>
          ...
          ${sheet.axis.values.map((v, i) => {
            const colorClass = axisClass(sheet.axis, i);
            return `<th>${colorClass ? `<span class="axis-label ${colorClass}">${esc(v)}</span>` : esc(v)}</th>`;
          }).join('')}
        </tr></thead>
        <tbody>${rows}</tbody>
      </table>
          ...
          <table class="pattern-table">
            ...
            <tbody>
              ${rows.map((r) => `<tr>${r.map((c, i) => {
                const colorClass = i === 0 ? axisClassForLabel(sheet.axis, c) : '';
                return `<td${i === accentColumn ? ' class="accent-col"' : ''}>${colorClass ? `<span class="axis-label ${colorClass}">${esc(c)}</span>` : esc(c)}</td>`;
              }).join('')}</tr>`).join('')}
            </tbody>
          </table>
        </div>`;
...
function mountQuiz(sheet, root) {
  ...
  function setAxisLabel(label, index = null) {
    els.axis.className = 'q-axis';
    if (index !== null) {
      const colorClass = axisClass(sheet.axis, index);
      if (colorClass) els.axis.classList.add(colorClass);
    }
    els.axis.textContent = label;
  }
  ...
  function pickQuestion() {
    ...
    const choices = currentPool.length > 1
      ...
      : currentPool;
    return weightedPick(choices, (question) => (
      quizConfig.axisWeights[sheet.axis.ids[question.axisIndex]] ?? 1
    ));
  }
  ...
  function showQuestion({ focusAnswer = true } = {}) {
    ...
    setAxisLabel(
      multipleChoice ? 'Choose the English meaning' : currentQ.axisLabel,
      multipleChoice ? null : currentQ.axisIndex,
    );
  ...
  function renderOptions() {
    ...
    els.axisOptions.innerHTML = sheet.axis.values.map((label, index) => {
      ...
      <label>
        ...
        <span class="axis-label${colorClass ? ` ${colorClass}` : ''}">${esc(label)}</span>
      </label>`;
    }).join('');
  }
