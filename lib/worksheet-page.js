...
function renderWorksheet(sheet, opts) {
  ...
  return `
    <section class="paper">
      ...
      ${blanks.length ? renderBlanks(sheet, blanks) : ''}
...
function renderBlanks(sheet, questions) {
      ...
      ${questions.map((q, i) => `
        <li class="blank-item">
          ...
          <span class="blank-prompt">
            ...
            <span class="blank-axis">(<span class="axis-label ${axisClass(sheet.axis, q.axisIndex)}">${esc(q.axisShortLabel)}</span>)</span>
      ...
      ${items.map((item, i) => `
        <div class="grid-item">
          ...
          <table class="grid-table">
            <thead><tr>${sheet.axis.shortValues.map((v, i) => {
          ...
          ${blanks.map((q, i) => `
            <li class="key-item">
              ...
              <span>
                ...
                <span class="key-term">${esc(q.item.term)} · <span class="axis-label ${axisClass(sheet.axis, q.axisIndex)}">${esc(q.axisShortLabel)}</span></span>
              </span>
            </li>`).join('')}
        </ol>` : ''}
      ${tableItems.length ? `
...
function renderReference(sheet, opts) {
  ...
  return `
    <section class="paper">
      ...
      <div class="ref-flow">
        ${patternTables.map((section) => patternBlock(section, sheet)).join('')}
...
function patternBlock(section, sheet) {
    ...
    <table class="ref-table-print">
      ...
      <tbody>
        ${rows.map((r) => `<tr>${r.map((c, i) => {
          ...
          return `<td${i === 0 ? ' class="ref-label"' : ''}>${colorClass ? `<span class="axis-label ${colorClass}">${esc(c)}</span>` : esc(c)}</td>`;
        }).join('')}</tr>`).join('')}
      </tbody>
    </table>`);
}
  ...
  const body = `
    <table class="ref-table-print">
      <thead><tr>
        ...
        ${sheet.axis.shortValues.map((v, i) => {
          const colorClass = axisClass(sheet.axis, i);
          return `<th>${colorClass ? `<span class="axis-label ${colorClass}">${esc(v)}</span>` : esc(v)}</th>`;
        }).join('')}
      </tr></thead>
