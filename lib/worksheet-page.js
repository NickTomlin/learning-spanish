/**
 * The site's one worksheet page. Which sheet it prints is just another option,
 * so every configuration lives at the same page, driven entirely by the URL:
 *
...
export function mountWorksheetPage(rawSheet, root, { catalog = [] } = {}) {
  ...
  document.title = `${sheet.title} worksheet · Español`;
  ...
  function update(patch, { rerenderControls = false } = {}) {
    ...
    paperEl.innerHTML = renderWorksheet(sheet, opts);
...
function controlsMarkup(sheet, opts, catalog) {
  ...
  return `
    ...
    ${catalog.length > 1 ? `
    <nav class="ws-sheet-tabs" aria-label="Worksheet">
      ${catalog.map((s) => `
    ...
    <p class="ws-hint">This URL regenerates the exact same worksheet — bookmark it, or change the seed for a fresh set.</p>`;
}
